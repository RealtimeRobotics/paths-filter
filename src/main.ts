...
async function getChangedFilesFromPush(): Promise<string[] | null> {
  ...
  const baseInput = git.trimRefs(
    core.getInput("base", { required: false }) || push.repository.default_branch
  );
...
async function getChangedFilesFromGit(ref: string): Promise<string[]> {
  core.debug(
    "Fetching base branch and using `git diff-index` to determine changed files"
  );
